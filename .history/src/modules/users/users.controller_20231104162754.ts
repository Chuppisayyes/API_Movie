...
export class UsersController {
  ...
  async layThongTinNguoiDung(@Param('id') id: string) {
    try {
      if (!id) {
        throw new BadRequestExceptionxception('Thiếu tham số ID hoặc giá trị ID không hợp lệ');
      }
      ...
      const user = await this.usersService.layThongTinNguoiDung(userId);
      ...
      console.error(error); // In lỗi ra console để xem lỗi cụ thể
