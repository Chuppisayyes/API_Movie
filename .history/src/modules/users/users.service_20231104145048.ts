...
export class UsersService {
  ...
  async thongTinTaiKhoan(userId: number) {
    try {
      if (userId) {
        return { tai_khoan: userId }; // Trả về thông tin người dùng dựa trên 'tai_khoan' đã được giải mã từ token
      } else {
        throw new HttpException(userId + 'Người dùng không tồn tại', 404);
      }
    } catch (err) {
