  ...
  async timKiemNguoiDung(body) {
    try {
      ...
      const users = await prisma.nguoiDung.findMany({
        where: {
          OR: [
            { tai_khoan: { contains: tai_khoan } }, // Tìm theo tài khoản
            { ho_ten: { contains: ho_ten } }, // Tìm theo họ tên
            { email: { contains: email } }, // Tìm theo email
            { so_dt: { contains: so_dt } }, // Tìm theo số điện thoại
          ],
