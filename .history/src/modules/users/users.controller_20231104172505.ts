...
export class UsersController {
  ...
  async thongTinTaiKhoan(@Req() req) {
    const userId = req.user.tai_khoan; // Lấy 'tai_khoan' từ thông tin người dùng trong token
    const userInfo = await this.usersService.thongTinTaiKhoanByToken(userId);
    return userInfo;
  }
  ...
  async layThongTinNguoiDung(@Param('id') id: string) {
      const user = await this.usersService.layThongTinNguoiDung(id.toString());
      if (!user) {
  ...
  @ApiBody({
    ...
    type: TimKiemNguoiDungDto, // Sử dụng TimKiemNguoiDungDto thay vì NguoiDung
  })
  @ApiBearerAuth()
  @UseGuards(AuthGuard("jwt"))
  @Post('/ThemNguoiDung')
  themNguoiDung(@Body() body: TimKiemNguoiDungDto) { // Sử dụng TimKiemNguoiDungDto thay vì NguoiDung
