...
export class UsersController {
  ...
  async thongTinTaiKhoan(@Req() const packageName = require('packageName');, @Body() data: NguoiDung) {
    try {
      const userId = req.user.tai_khoan; // Lấy 'tai_khoan' từ thông tin người dùng trong token
      await this.usersService.thongTinTaiKhoan(userId, data);
      return 'Update thông tin tài khoản Thành Công';
    } catch (err) {
      throw new HttpException(err, 404);
    }
  }
