...
export class UsersService {
  ...
  async layThongTinNguoiDung(id: string) {
    try {
      ...
      if (isNaN(userId)) {
        throw new HttpException('ID không hợp lệ', 400); // Trả về lỗi BadRequest nếu ID không hợp lệ
      }
      ...
      if (!user) {
        throw new NotFoundException('Không tìm thấy người dùng'); // Trả về lỗi Not Found nếu không tìm thấy người dùng
      }
  
      return user;
    } catch (error) {
      throw new HttpException('Lỗi không xác định', 500); // Trả về lỗi Internal Server Error nếu có lỗi không xác định
    }
  }
